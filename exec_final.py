...
class Writer(object):
	def __init__(self, msg):
		...
		self.retries = 6 #retry per second
	...
	def postMsg(self):
                try:
                        ...
                        if str(req.getcode()) != "200" and str(req.read()) != "OK":
                                print "Err http code" + str(req.getcode())
                                time.sleep(1)
                                self.retries -= 1
                                if self.retries > 0:
                                        return self.postMsg()
                                else:
                                        return "NSQ cannot connect."
                        else:
                                return str(req.read()) + "," + str(req.getcode())
                except Exception, e:
                        print e
