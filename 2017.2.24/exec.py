...
class Reader(threading.Thread):
	...
	def __init__(self, t_name, queue, addr='x', topic="a", channel="c",lookupd_poll_interval=5,max_in_flight=1):
...
class Writer(threading.Thread):
	...
	def __init__(self, t_name, queue, addr='x' ,topic="mysql_response"):
		threading.Thread.__init__(self,name=t_name)
		self.data = queue
		self.addr = addr
		self.topic = topic
...
class Execute(threading.Thread):
	...
	def _download(self, RES):
		try:
			curl = "wget --http-user=" + RES['user'] + " --http-passwd=" + RES['passwd'] + " -P /root/monitor/scripts " + RES['source']
			(status,res) = commands.getstatusoutput(curl)
		except Exception, e:
			pass
		sys.stdout.flush()
		return True
