...
class Reader(threading.Thread):
	...
	def __init__(self, t_name, queue, addr='xxxx', topic="a", channel="c",lookupd_poll_interval=15,max_in_flight=50):
...
def foo():
	...
	try:
		...
		if lookupd_address:
			...
			r = Reader('Reader.', readQueue, None, request_topic)

		if nsqd_address:
			w = Writer('Writer.', writeQueue, nsqd_address, response_topic)
		else:
			w = Writer('Writer.', writeQueue, None, response_topic)
