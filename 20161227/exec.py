...
class Reader(threading.Thread):
	...
	def __init__(self, t_name, queue, addr='lookupd-address', topic="xx", channel="c",lookupd_poll_interval=15,max_in_flight=50):
...
class Writer(threading.Thread):
	...
	def __init__(self, t_name, queue, addr='nsqd-address' ,topic="Mysql_Monitor"):
		threading.Thread.__init__(self,name=t_name)
		self.data = queue
		self.addr = addr
		self.topic = topic
...
def foo():
	...
	try:
		...
		if lookupd_address:
			r = Reader('Reader.', readQueue, lookupd_address)
		else:
			r = Reader('Reader.', readQueue)

		if nsqd_address:
			w = Writer('Writer.', writeQueue, nsqd_address)
		else:
